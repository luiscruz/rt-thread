...
/* change oled type to 0xF01 or 0xF02 */
static rt_uint32_t wisechip_oled=0x0F01;
#ifdef RT_USING_FINSH
#include <finsh.h>
FINSH_VAR_EXPORT(wisechip_oled, finsh_type_uint, wisechip oled type 0x0f01 is default);
#endif
...
{
	if(wisechip_oled == 0xF02){
		//nop
	}else/*	if(wisechip_oled == 0xF01)*/{
		int i=0;
		uint8_t params[]={
		0x50,
...
unsigned int wisechip_f01_init(unsigned int lcdid)
{
	...
	if(wisechip_oled == 0xF02){
		Set_Driving_Current(0x45,0x34,0x33);	// Set Driving Current of Red
							// Set Driving Current of Green
							// Set Driving Current of Blue
		Set_Gray_Scale_Table();			// Set Pulse Width for Gamma Table
		Set_Precharge_Period(0x04,0x05,0x05);	// Set Pre-Charge Time of Red
							// Set Pre-Charge Time of Green
							// Set Pre-Charge Time of Blue
		Set_Precharge_Current(0x9D,0x8C,0x57);	// Set Pre-Charge Current of Red
